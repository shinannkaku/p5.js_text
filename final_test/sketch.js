...
function drawSnowflakes() {
    for (let i = 0; i < Snowflakes.length; i++) {
      ...
      beginShape();
        for (let j = 0; j < 6; j++) { // 五邊形的點數為5 for迴圈用來創建多邊形的各個頂點
          let angle = TWO_PI * j / 6; //計算每個頂點的角度 TWO_PI:360度(將其分成了12份 每份為30度)
          ...
          if (snowflake.y > height) {
            ...
            snowflake.x= random(-50, 0); //重新設置在畫布上方的位置
            snowflake.y = random(width); //隨機水平位置
          }
        }
      }
      ...
      /*function generateRandomShape() {
        let x = random(width);
        let y = random(height);
        let radius = random(40, 100); // 隨機大小
      
        let numVertices = int(random(3, 10)); // 隨機邊數
        let transparency = 200;
        fill(255, transparency);
        noStroke();
        ...
        for (let i = 0; i < numVertices; i++) {
          ...
          let offset = random(-10, 10); // 隨機偏移
          let sx = x + cos(angle) * (radius + offset);
          let sy = y + sin(angle) * (radius + offset);
          vertex(sx, sy);
        }
        endShape(CLOSE);
      }
      ...
      function drawRandomShape() {
        // 移動雪花的位置(用來更新雪花的位置)
        // 暫時假設 weight 和 height 是全局變數
        weight += 1; // 假設這裡是某個雪花的速度
        height += 1 ;// 假設這裡是某個雪花的速度
      
        // 如果雪花超出畫布，重新放置到畫布上方
        if (height > windowHeight) {
          // 重新設置雪花位置到畫布上方，隨機水平位置
          height = random(-50, 0); // 重新設置在畫布上方的位置
          weight = random(windowWidth); // 隨機水平位置
        }
      }*/
